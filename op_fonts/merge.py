"""fontTools.merge wrapper — merges multiple subset TTFs into one."""
...
def _normalize_upm(font_path: Path, target_upm: int) -> Path:
    ...
    out_path = font_path.with_suffix(".scaled.ttf")
...
def merge_fonts(
    ...
    """Merge multiple TTF files into a single font.
    ...
    # Convert CFF→TTF if needed, normalize UPM, ensure GSUB exists
    target_upm = TTFont(font_paths[0])["head"].unitsPerEm
    TTFont(font_paths[0]).close()

    processed: list[Path] = []
    for p in font_paths:
        converted = _ensure_quadratic(p)
        converted = _normalize_upm(converted, target_upm)
        _ensure_gsub(converted)
        # Strip tables that will be dropped — avoids merge conflicts
        if drop_tables:
